...
/**
 * Max images fetched/rasterized at once. Unbounded parallelism floods the
 * browser's on-disk cache (disk I/O hits 100% on image-heavy pages); 6 matches
 * a browser's typical per-host connection limit.
 * ponytail: fixed pool of 6, make it an option if some page needs tuning.
 */
const IMAGE_FETCH_CONCURRENCY = 6;

/**
 * Run async task factories at most `limit` at a time, feeding each result to
...
export class ResourceResolver {
  ...
  /** Deferred image-fetch tasks, run with bounded concurrency in getBlobMap. */
  imageTasks: Array<() => Promise<[string, AssetEntry]>> = [];

  /** URLs already queued as image tasks (dedupe). */
  queuedImageUrls = new Set<string>();
  ...
  addImage(url: string, sourceElement?: HTMLImageElement): void {
    if (!url || this.queuedImageUrls.has(url)) return;
    this.queuedImageUrls.add(url);

    // Defer the fetch instead of starting it now: getBlobMap runs these with
    // bounded concurrency so we never flood the disk cache with parallel loads.
    if (this.options.skipRemoteAssetSerialization && isRemoteUrl(url)) {
      this.imageTasks.push(async () => [url, { url, blob: null }]);
    } else {
      this.imageTasks.push(async () => {
        try {
          return [url, await fetchImageAsBlob(url, sourceElement)];
        } catch (error) {
          return [url, { url, blob: null, error: String(error) }];
        }
      });
    }
  }
  ...
  async getBlobMap(): Promise<Map<string, AssetEntry>> {
    ...
    // Eager promises: canvas/video frames (few, local — no disk-cache flood).
    for (const [url, promise] of this.promises.entries()) {
      blobMap.set(url, await promise);
    }

    // Images: run with bounded concurrency instead of all at once.
    await runWithConcurrency(this.imageTasks, IMAGE_FETCH_CONCURRENCY, ([url, entry]) => {
      blobMap.set(url, entry);
    });
...
function collectImageElement(element: Element, assetCollector: ResourceResolver): void {
  ...
  if (element instanceof view.HTMLImageElement) {
...
function collectVideoElement(element: Element, assetCollector: ResourceResolver): void {
  const view = element.ownerDocument?.defaultView ?? window;
  if (!(element instanceof view.HTMLVideoElement)) return;
