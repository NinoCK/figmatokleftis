...
function copySvgComputedStyles(source: Node, target: Node): void {
  // Realm-safe element check: SVG nodes inside a same-origin iframe are not
  // instances of the top window's Element, which would otherwise skip styling.
  const view = source.ownerDocument?.defaultView ?? window;
  if (!(source instanceof view.Element) || !(target instanceof view.Element)) return;
