...
export function isNodeVisible(element: Element): boolean {
  ...
  if (element instanceof view.HTMLScriptElement) return false;
  if (
...
export function getTextRect(nodeOrNodes: Node | Node[]): { x: number; y: number; width: number; height: number; lineCount: number } {
  // Use the node's own document so text inside a same-origin iframe measures
  // against the correct document rather than the top one.
  const firstNode = Array.isArray(nodeOrNodes) ? nodeOrNodes[0] : nodeOrNodes;
  const range = (firstNode.ownerDocument ?? document).createRange();
  ...
  const ancestorView = ancestor.ownerDocument?.defaultView ?? window;
  const isVertical =
    ancestor instanceof ancestorView.HTMLElement
...
export function getElementAttributes(element: Element): Record<string, string> {
  ...
  const view = element.ownerDocument?.defaultView ?? window;
  if (element instanceof view.HTMLVideoElement && element.poster) {
    attrs.poster = element.poster;
  }
  if (
    (element instanceof view.HTMLImageElement || element instanceof view.HTMLVideoElement) &&
    element.currentSrc
  ) {
  ...
  if (element instanceof view.HTMLInputElement && attrs.type == null) {
