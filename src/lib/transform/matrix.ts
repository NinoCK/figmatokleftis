...
export function getElementDimensions(element: Element | Text): { width: number; height: number } {
  ...
  // Resolve constructors from the node's own realm (iframe-safe instanceof).
  const view = element.ownerDocument?.defaultView ?? window;

  if (element instanceof view.HTMLElement) {
    width = element.offsetWidth;
    height = element.offsetHeight;
  } else if (element instanceof view.SVGSVGElement) {
  ...
  } else if (element instanceof view.SVGGraphicsElement) {
    const bbox = element.getBBox();
    width = bbox.width;
    height = bbox.height;
  } else if (element instanceof view.MathMLElement) {
    const clientRect = element.getBoundingClientRect();
    width = clientRect.width;
    height = clientRect.height;
  } else if (element instanceof view.Text) {
...
export function getElementRect(element: Element | Text, computedStyle: CSSStyleDeclaration, combinedTransform: DOMMatrix | undefined | null): ElementRect {
  const view = element.ownerDocument?.defaultView ?? window;
  const boundingRect = element instanceof view.Element
    ? element.getBoundingClientRect()
    : (() => { const r = (element.ownerDocument ?? document).createRange(); r.selectNode(element); const rect = r.getBoundingClientRect(); r.detach(); return rect; })();
