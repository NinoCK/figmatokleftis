(() => {
  ...
  const iconPaths: Record<string, string[]> = {
    ...
    clock: [
      "M8 1.5a6.5 6.5 0 1 0 0 13 6.5 6.5 0 0 0 0-13Zm0 1a5.5 5.5 0 1 1 0 11 5.5 5.5 0 0 1 0-11Z",
      "M7.5 4a.5.5 0 0 1 1 0v3.7l2.3 1.35a.5.5 0 1 1-.5.86l-2.55-1.5A.5.5 0 0 1 7.5 8V4Z",
    ],
  };
  ...
  title.textContent = "Figmatokleftis";
  ...
  actions.append(btnScreen, btnSelect, btnDelay);
  ...
  // Delayed capture: gives you time to open a dropdown/menu (which would
  // otherwise close the moment you click a toolbar button) before the snapshot.
  async function captureAfterDelay(seconds: number): Promise<void> {
    captureAborted = false;
    setLoading(true);
    for (let s = seconds; s > 0; s--) {
      if (captureAborted || !host.isConnected) { setLoading(false); return; }
      showStatus(`Capturing in ${s}… open your menu now`, false);
  ...
  function setLoading(loading: boolean): void {
    ...
    btnDelay.classList.toggle("disabled", loading);
  }
